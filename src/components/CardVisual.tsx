...
/**
 * Презентационная визитка. Используется и в предпросмотре (из Zustand),
 * и на публичной странице /p/[slug] (данные из базы).
 */
export function CardVisual({
  ...
  // На публичной визитке логотип и фон показываем, если они есть.
          ...
          {isOffline ? (
            ...
            <>
              ...
              <Section title="Личные контакты">
                ...
                />
                {c.personal.customButtons.map((btn) => (
                  <LinkButton
                    key={btn.id}
                    label={btn.label}
    ...
    <>
      {(b.hours || b.address) && (
        <Section title="Информация">
          ...
          {b.address && (
            <ContactRow icon={<MapPin size={16} />} value={b.address} />
          )}
        </Section>
      )}
